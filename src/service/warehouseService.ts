...
export async function saveShelvesService(warehouseId: string, zoneId: string, 
    shelfIdAndNames:{[shelfId: string]: string}): Promise<IMessage> {
...
async function findDuplicateShelves(shelves: IShelf[]): Promise<IMessage> {
    ...
    for (let i=0; i<shelves.length; i++) {
        ...
        if (dbRecord) {
            duplicateShelvesMsg.push("Shelf name '" + shelves[i].name + "' already exists in Zone " + dbRecord.zoneId + ".");
        }
    }
    
    if (duplicateShelvesMsg.length !== 0) return { status: true, message: duplicateShelvesMsg }
